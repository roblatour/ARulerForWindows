...
Public Module modKeySend

    Friend gSendingKeyesIsRequired As Boolean = False
    ...
    Friend Sub SendKeysToTargetWindow(ByVal KeysToSend As String)
        ...
        Try
            ...
            'updated here for A Ruler for Windows, don't need the date time update so that subroutine is commented out and the code below adjusted accordingly
            'Dim TransmissionString As String = UpdateWithDateAndTime(KeysToSend)
            'SendTheTransmissionString(TransmissionString)
            SendTheTransmissionString(KeysToSend)
    ...
    'Friend Sub SendWinUpArrow_MaximizeWindow()
    ...
    'Friend Sub SendWinDownArrow_MinimizeWindow()
    ...
    '    ' KeyPress(VirtualKeyCode.Invalid) 'seems to be required to clear out a buffer or something like that.

    '    KeyUp(VirtualKeyCode.CAPSLOCK)
    '    KeyUp(VirtualKeyCode.SCROLLLOCK)
    '    KeyUp(VirtualKeyCode.NUMLOCK)
    '    KeyUp(VirtualKeyCode.LCONTROL)
    '    KeyUp(VirtualKeyCode.RCONTROL)
    '    KeyUp(VirtualKeyCode.LMENU)
    '    KeyUp(VirtualKeyCode.RMENU)
    '    KeyUp(VirtualKeyCode.LWIN)
    '    KeyUp(VirtualKeyCode.RWIN)
    '    KeyUp(VirtualKeyCode.LSHIFT)
    '    KeyUp(VirtualKeyCode.RSHIFT)

    'End Sub

    <Obfuscation(Feature:="virtualization", Exclude:=False)>
    Friend Sub SendTheTransmissionString(ByVal TransmitString As String)
        ...
        'If OriginalCAPSLock Then KeyPress(VirtualKeyCode.CAPSLOCK)
        'If Not OriginalNumbLock Then KeyPress(VirtualKeyCode.NUMLOCK)
        'If OriginalScrollLock Then KeyPress(VirtualKeyCode.SCROLLLOCK)
...
DoneWithThisKey:
        ...
        If OriginalCAPSLock Then
            If gCapsLock Then
            Else
                KeyPress(VirtualKeyCode.CAPSLOCK)
            End If
        Else
            If gCapsLock Then
                KeyPress(VirtualKeyCode.CAPSLOCK)
        ...
        If gNumbLock Then
            ...
            Else
                KeyPress(VirtualKeyCode.NUMLOCK)
            End If
        Else
            If (Control.IsKeyLocked(Keys.NumLock)) Then
                KeyPress(VirtualKeyCode.NUMLOCK)
        ...
        If gScrollLock Then
            ...
            Else
                KeyPress(VirtualKeyCode.SCROLLLOCK)
            End If
        Else
            If (Control.IsKeyLocked(Keys.Scroll)) Then
                KeyPress(VirtualKeyCode.SCROLLLOCK)
    ...
    Friend Sub GetSimpleKeysVirtualKeyCode(ByVal InputString As String, ByRef VirtKeyCode As VirtualKeyCode, ByRef ShiftRequired As Boolean, ByRef AltRequired As Boolean, ByRef CtrlRequired As Boolean)
        ...
        For x As Integer = 1 To MyKeyBoardKeys.Length - 1
    ...
    Friend Sub SetPriority(ByVal Prioirty As ProcessPriorityClass)
    ...
    Friend MyKeyBoardKeys(255) As MyKeyBoard
    ...
    Friend Sub GetLockKeyStates()
        ...
        'Dim WaitUntilDone As Boolean = System.Windows.Application.Current.Dispatcher.Invoke(Function()
        '                                                                                        gCapsLock = My.Computer.Keyboard.CapsLock
        '                                                                                        gNumbLock = My.Computer.Keyboard.NumLock
        '                                                                                        gScrollLock = My.Computer.Keyboard.ScrollLock
        '                                                                                        Return True
